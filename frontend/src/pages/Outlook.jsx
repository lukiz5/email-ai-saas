...
import { getOutlookStatus, getOutlookAuthUrl, listOutlookMessages, summarizeEmails } from '../lib/api';
...
const Outlook = () => {
  ...
  const handleConnect = async () => {
    try {
      const { auth_url } = await getOutlookAuthUrl();
      window.location.href = auth_url;
    } catch (error) {
      alert('Error connecting to Outlook. Please try again.');
    }
  };
  ...
  const loadMessages = async () => {
    setLoading(true);
    try {
      const data = await listOutlookMessages(20, 'inbox');
      setMessages(data.messages || []);
    } catch (error) {
      alert('Error loading Outlook messages');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      const result = await summarizeEmails(emailsToSummarize, 'outlook');
  ...
  if (!isPro) {
    return (
      <div className="max-w-4xl mx-auto">
        <Paywall feature="Outlook integration" />
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <Loader size="lg" text="Loading Outlook..." />
      </div>
    );
  }
  ...
  if (!connected) {
    return (
      <div className="max-w-2xl mx-auto mt-12">
        <Card title="Connect Outlook" subtitle="Authorize access to your Microsoft 365 account">
          <div className="text-center py-8">
            <MessageSquare size={64} className="mx-auto mb-6 text-gray-400" />
            <p className="text-gray-600 mb-6">
              Connect your Outlook/Microsoft 365 account to automatically fetch and summarize your emails
            </p>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Outlook</h1>
